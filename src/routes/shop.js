...
router.get('/products', async (req, res) => {
  try { 
    const products = await global.db.getActiveProducts(); 
    res.json(products); 
  }
  catch (error) { 
...
router.get('/leaderboard', authenticateToken, async (req, res) => {
  try {
    ...
    const rows = await global.db.query(`SELECT id, username, coins, user_level, total_checkins, total_invites FROM users WHERE is_active = 1 ORDER BY ${orderBy} LIMIT 50`);
    res.json(rows);
  } catch (error) { 
    res.status(500).json({ error: error.message }); 
  }
